...
function FiscalsManagementPage({ token, onBack }) {
  ...
  const openPasswordModal = (fiscal) => {
    setPasswordModalId(fiscal.id);
    setPasswordValue('');
  };
  ...
  const toggleResolved = async (fiscal) => {
    if (!fiscal.feedback_id) {
      return;
    }
    ...
    try {
      await axios.patch(
        `${API_URL}/admin/investigadores/${fiscal.id}/resuelto`,
        { resuelto: !fiscal.feedback_resuelto },
        { headers: authHeaders }
      );

      setSuccess(
        !fiscal.feedback_resuelto
      ...
      <header className="border-b border-cyan-500/20 bg-panel-dark px-8 py-4 backdrop-blur-md">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="font-mono text-xs text-cyan-300/70">Crear cuentas, cambiar contraseña y reabrir la prueba</p>
          </div>
          <button
            type="button"
            onClick={onBack}
      ...
      <main className="p-8">
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Desde aquí puedes administrar acceso y estado de la prueba.</p>
          </div>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            {fiscals.map((fiscal) => (
              <div key={fiscal.id} className="rounded-xl border border-cyan-500/20 bg-slate-950/70 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-mono text-lg text-slate-100">{fiscal.nombre}</p>
                    <p className="mt-1 text-xs text-slate-400">{fiscal.email}</p>
                  </div>
                  <span
                  ...
                  >
                    {fiscal.feedback_id ? (fiscal.feedback_resuelto ? 'Resuelto' : 'Desbloqueado') : 'Sin prueba'}
                  </span>
                </div>
                ...
                <div className="mt-4 space-y-2 rounded-lg border border-slate-500/20 bg-slate-900/60 p-3 text-xs text-slate-300">
                  <p className="flex items-center gap-2">
                    <FiUsers size={13} />
                    Estado: {fiscal.activo ? 'Activo' : 'Inactivo'}
                  </p>
                  <p className="flex items-center gap-2">
                    <FiShield size={13} />
                    Prueba: {fiscal.feedback_id ? `Puntaje ${fiscal.feedback_puntaje}%` : 'Aún no presentada'}
                  </p>
                  <p className="flex items-center gap-2">
                    <FiRefreshCw size={13} />
                    Esperadas: {fiscal.feedback_expected_total || 0} | Trazadas: {fiscal.feedback_user_total || 0}
                  </p>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => openPasswordModal(fiscal)}
                  ...
                  <button
                    type="button"
                    onClick={() => toggleResolved(fiscal)}
                    disabled={!fiscal.feedback_id || saving}
      ...
      {passwordModalId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 py-8">
          <div className="w-full max-w-lg rounded-2xl border border-cyan-500/30 bg-slate-900/95 p-6 shadow-[0_0_40px_rgba(8,145,178,0.2)]">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="font-mono text-lg font-semibold tracking-[0.2em] text-cyan-200">CAMBIAR CONTRASEÑA</h3>
              <button
                type="button"
...
export default FiscalsManagementPage;
